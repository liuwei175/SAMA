...
def load_split_results() -> pd.DataFrame:
    ...
    keys = ["dataset", "model", "epsilon", "alpha0", "lambda_l1", "sampling_policy"]
...
def make_loss_table(summary: pd.DataFrame) -> pd.DataFrame:
    robust = summary.pivot_table(
        index=["epsilon", "N"],
        columns="model",
        values="te_robust_loss_mean",
        aggfunc="first",
    )
    cvar = summary.pivot_table(
        index=["epsilon", "N"],
        columns="model",
        values="te_cvar_loss_mean",
        aggfunc="first",
    )
